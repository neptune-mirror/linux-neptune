...
/* Set device speed to the maximum supported */
...
static int amd_set_spi_freq(struct amd_spi *amd_spi, u32 speed_hz)
{
	...
	for (i = 0; i < ARRAY_SIZE(amd_spi_freq); i++) {
		if (speed_hz >= amd_spi_freq[i].speed_hz) {
			...
			enable_val = (amd_spi_freq[i].enable_val << AMD_SPI_ALT_SPD_SHIFT) & AMD_SPI_ALT_SPD_MASK;
			amd_spi_setclear_reg32(amd_spi, AMD_SPI_ENA_REG, enable_val, AMD_SPI_ALT_SPD_MASK);

			if (amd_spi->speed_hz == AMD_SPI_MAX_HZ)
				amd_spi_setclear_reg32(amd_spi, AMD_SPI_ENA_REG, 1, AMD_SPI_SPI100_MASK);

			if (amd_spi_freq[i].spd7_val) {
				spd7_val = (amd_spi_freq[i].spd7_val << AMD_SPI_SPD7_SHIFT) & AMD_SPI_SPD7_MASK;
				amd_spi_setclear_reg32(amd_spi, AMD_SPI_SPEED_REG, spd7_val, AMD_SPI_SPD7_MASK);
			}
